...
public interface GroupItemDAO{
    ...
    @Transaction
    @Query("SELECT `group`.group_id,`group`.group_name,`group`.group_img,`group`.group_admin,`group`.group_complete FROM `group` JOIN UserGroupCrossRef ON UserGroupCrossRef.group_id=`group`.group_id WHERE UserGroupCrossRef.user_id=:val AND `group`.group_complete = 0 ORDER BY `group`.group_id DESC ")
